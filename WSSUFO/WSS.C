...
namespace Foam
{
    defineTypeNameAndDebug(WSS, 0);
}


// * * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * //

void Foam::WSS::writeFileHeader(const label i)
{
    ...
    phiName_("phi")
{
    ...
    if (active_)
    {
        ...
        mesh.objectRegistry::store(WSSPtr);
    }
...
void Foam::WSS::read(const dictionary& dict)
{
    if (active_)
    {
        ...
        else
        {
            ...
            forAllConstIter(labelHashSet, patchSet_, iter)
            {
                ...
                {
                    WarningIn("void WSS::read(const dictionary&)")
...
void Foam::WSS::execute()
{
    ...
    if (active_)
    {
        ...
        {
            FatalErrorIn("void Foam::WSS::execute()")
